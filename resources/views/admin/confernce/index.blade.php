        ...
        <div class="section-body">
            <div class="row">
                ...
                <div class="card">
                    ...
                    <a href="{{ route('admin.conference.create') }}" class="btn btn-icon icon-left btn-info btn-list-add">
                    ...
                    <div class="table-responsive">
                        ...
                        <tbody>
                            @if($confernce)
                            @foreach($confernce as $key => $value)
                                <tr>
                                    <td>{{ ($key+1) }}</td>
                                    ...
                                    <td>
                                        <a href="{{ route('admin.conference.edit', $value->id) }}" title="Update" class="btn btn-primary">
                                            <i class="fas fa-edit"></i>
                                        </a>
                                        <a href="javascript:void(0)" data-id="{{ $value->id }}" title="Delete" class="btn btn-danger btn-delete-row">
            ...
            $('.btn-delete-row').on('click', function(){
               let id = $(this).data('id');
                swal({
                    ...
                    if (willDelete) {
                        $.ajax({
                            url: "{{ url('admin/delete-confrence') }}"+"/"+id,
                            type:'get',
                            data: {
                                "_token": "{{ csrf_token() }}",
                            },
