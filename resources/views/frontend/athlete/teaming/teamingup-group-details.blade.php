...
<div class="teamingupdetails_l_member">
	<h3>Members</h3>
	<ul>
		<li><a href="{{ route('athlete.teamingup-group-details.add-mamber',$group_detail->id) }}"><img src="{{ asset('public/frontend/athlete/images/add_member.png') }}" alt="add_member"/> Add members</a></li>
		<!-- <li><a href="javascript:void(0);" data-toggle="modal" data-target="#invitealink"><img src="{{ asset('public/frontend/athlete/images/invite_link_icon.png') }}" alt="invite_link_icon"/> Invite via link</a>
		</li> -->
		<li>
		...
		<li><a href="{{ route('athlete.edit-teamingup-group',$group_detail->id) }}"><img src="{{ asset('public/frontend/athlete/images/edit_icon.png') }}" alt="edit_group"/> Edit Group</a></li>
    ...
    <div class="modal-dialog filterpopup" role="document">
        <div class="modal-content">
            <div class="modal-body">
                ...
                <div class="container-fluid">
					<form action="{{ url('athlete/add-group-library/').'/'.$group_detail->id }}" method="POST" id="form-workout-library">
						@csrf
    ...
    <div class="invitepopupbox">
    	...
    	<div class="invitepopupbox_r popupsharemodal_r">
    		...
    		<?php $name_slug= str_replace(' ', '-', @$group_detail->group_name); ?>
    		 ...
    		 <p id="link-text"><?php echo url('/athlete/teamingup-group-details/'. $group_detail->id.'/?group-name='.@$name_slug);?></p>
    		
    	</div>
    	<div class="clr"></div>
    </div>
    <input type="hidden" name="hidden_link" value="<?php echo url('/athlete/teamingup-group-details/'. $group_detail->id);?>" id="group_link">
    <ul>
		...
		.then((willDelete) => {
			if (willDelete) {
				$.ajax({
					...
					url: "{{ url('athlete/delete-member') }}"+"/"+id,
		...
		.then((willDelete) => {
			if (willDelete) {
				$.ajax({
					...
					url: "{{ url('athlete/delete-group') }}"+"/"+id,
			...
			if (willDelete) {
				$.ajax({
					...
					success: function(data) {						
						swal(data.message, 'success')
						.then( () => {
							window.location.href = baseUrl+"/athlete/teamingup-group";
							//location.reload();
						});
					}
				});
		...
		.then((willAdmin) => {
			if (willAdmin) {
				$.ajax({
					...
					url: "{{ url('athlete/create-group-admin') }}",					
					type:'POST',
					data: {
						 userId: userId,
						 groupId: groupId,
...
$(document).ready(function() {
	...
	$('#addhighlightsbtn').on('click', function() {
        ...
        $.ajax({
            url: "{{url('athlete/comparison-group-add')}}",
            type: 'POST',
            data: {
            ...
            success: function(data) {
                swal(data.message, 'Success', 'success')
                    .then(() => {
                        // location.reload();
						window.location.href = base_url+'/athlete/compare';
                    });
            }
        });
    });
