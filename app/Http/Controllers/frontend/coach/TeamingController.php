...
class TeamingController extends Controller
{
    ...
    {   
        ...
        return view('frontend.coach.teaming.index')
    ...
    {   
        ...
        // echo"<pre>"; print_r($category_library_id_array);exit;

        $allWorkoutCategory = WorkoutCategory::where('status', 1)->orderBy('is_aci_index', 'desc')->get(); 

        // echo "<pre>";
        // print_r($workout);exit;
        return view('frontend.coach.teaming.teamingup-group-details')
    ...
    public function addGroupLibrary($group_id, Request $request){
        ...
        return redirect('/coach/teamingup-group')->with('success','Group Updated Successfully');
    ...
    { 
       ...
       return view('frontend.coach.teaming.create-teamingup-group')
       ->with('workout', $workout)
       ->with('user', $user);                         
   }
...
/*****End filter*******/                        
return view('frontend.coach.teaming.edit-teamingup-group')
->with('workout', $workout)
->with('user', $user)
->with('group_detail', $group_detail)
...
public function updateTeamingGroup(Request $request, $id){
   ...
   /* if($group_user_id!=""){
    ...
    //return redirect('/coach/teamingup-group')->with('success','Group Updated Successfully');
    return redirect('coach/teamingup-group-details/'.$id.'?group-name='.$teaming->group_name)->with('success','Group Updated Successfully');
...
return view('frontend.coach.teaming.invite-member')
->with('user', $user)
...
{      
   ...
   if(DB::table('teamingup_group_users')->where('teaming_group_id', $groupId )->where('user_id', Auth()->user()->id)->delete()){ 
    ...
    public function createTeamingGroupAdd(Request $request){
        ...
        // echo 1;
        return redirect('/coach/teamingup-group')->with('success','Group Created Successfully');
        }else{
            return back()->with('error', 'Failed to add new group');
            // echo 0;
        }
