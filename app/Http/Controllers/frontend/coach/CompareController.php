...
class CompareController extends Controller
{
    ...
    {   
        ...
        $modalOpen = $request->input('modalOpen');
        $tabActive ='active';    
        $sports = [];
        $position = [];
        $states = '';
        ...
        // }
       // dd($group);
        return view('frontend.coach.compare.index')
        ...
        ->with('modalOpen', $modalOpen);
    }

    public function filterAthleteCompare(Request $request)
    {
        // echo "<pre>";
        // echo "hi"; 
        ...
        // dd($states);
        // $query = User::where('role_id', 1)->where('id', '!=', Auth()->user()->id)->with('address');
        $query =User::join('user_address','user_address.user_id', '=', 'users.id')->join('user_sport_positions','user_sport_positions.user_id', '=', 'users.id')->select('users.*','user_address.country_id')->groupBy('users.id');
        ...
        $athlete = User::where('role_id', 1)->where('id', '!=',Auth()->user()->id)->groupBy('id')
        ->with('address')
        ->get();
        }
        // print_r($athlete);
        ...
        $allathlete = [];
        $temp = [];
        foreach ($athlete as $key => $athlete_value) {
            // echo "<pre>";
            // print_r($athlete_value);
            // die;
            $country_name = Country::where('id', $athlete_value['country_id'])->first();
            // print_r($country_name);
            // die;
            $temp['id'] = $athlete_value['id'] ? $athlete_value['id'] : '' ;
            $temp['username'] =  $athlete_value['username'] ? $athlete_value['username'] : '' ;
            $temp['gender'] =  $athlete_value['gender'] ? $athlete_value['gender'] : '' ;
            $temp['profile_image'] =  $athlete_value['profile_image'] ? asset($athlete_value['profile_image']) : asset('public/frontend/athlete/images/defaultuser.jpg');
            $temp['country_id'] = $athlete_value['country_id'] ? $athlete_value['country_id']  : '';
            $temp['country_name'] = $athlete_value['country_id'] ? $country_name['name']  : '';
            $allathlete[] = $temp;
        }
        // print_r($allathlete);
        // die;
        echo json_encode($allathlete);
    }
    ...
    {   
        ...
        return view('frontend.coach.compare.add-athletes')
        ->with('athlete',$athlete);
    ...
    public function compareGroup($value='')
    {
        ...
        $view = view("frontend.coach.compare.group-compare")
    ...
    public function comparisonDetails($id='',$wrk_id='')
    {
        ...
        foreach($allHeaderCompare_array as $value){
            ...
            $workoutLibraryName = WorkoutLibrary::where('id', $workCatLib->workout_library_id)->first();
            $WorkoutCategoryName = WorkoutCategory::where('id', $workCatLib->workout_category_id)->first();
            
            // print_r($WorkoutCategoryName); die;
            $temp['Workout_category_library_id'] = $workCatLib->id;
            $temp['workout_category_id'] = $workCatLib->workout_category_id;
            $temp['workout_category_title'] = $WorkoutCategoryName->category_title;
        ...
        // echo "<pre>";
        // print_r($compare_value_user); die;
        ...
        $k = array_keys($compare_array12);
        $v = array_values($compare_array12);

        
        $key_val = array();
        foreach ($k as $key => $row)
        {
            $key_val[$key] = $row;
        }
        $new_key_value = array_multisort($key_val, SORT_ASC, $k);
        // print_r($k); die;
        // $rv = $compare_array12;

        $compare_array_final = array_combine($k, $compare_array12);
        // echo "<pre>";
        // print_r($compare_array_final);
        // die;
       
        return view('frontend.coach.compare.group-compare')
        ->with('workout_librarys',$workout_librarys)
        ->with('CompareGroup',$CompareGroup)
        ->with('compare_withs',$compare_array_final)->with('group_id', $id)->with('wrk_fill', $wrk_fill)->with('sort_value', $sort_value);
    }
    ...
    // public function comparisonGroupAdd(Request $request)
    // {
    //     // dd($request->all());
    //     // die;
    //     $allSelectedLibrary = $request->input('allSelectedLibrary');
    ...
    //         'group_comparison_name' => 'required',
    //     ]);
    //     $data['comparison_name'] = $request->comparison_name;
    //     $data['created_by']= Auth()->user()->id;
    //     $groupid = CompareGroup::insertGetId($data);
        
    //     if($groupid){            
    //         $group_user_id = $request->input('group_id');
    //     }

    //     $group_detail = Teaming::where('id',$group_user_id)->get();
    ...
    {
        // dd($request->all());
        // echo $request->input('comparison_name');
        $teamup_group_id = $request->input('group_id');
     
        $teamingWorkout_id = TeamingWorkout::where('teaming_group_id', $teamup_group_id)->get();
       
        $all_workout_id = [];
        foreach($teamingWorkout_id as $workout){
           $all_workout_id[] = $workout['workout_category_librarys_id'] ;
        }

        $allSelectedLibrary = implode(",",$all_workout_id);
        $allSelectedLibrary = $allSelectedLibrary.',';

        $data['comparison_name'] =  $request->input('comparison_name');
        $data['created_by']= Auth()->user()->id;
        $groupid = CompareGroup::insertGetId($data);
        ...
        // $group_detail = TeamingGroupUser::where('teaming_group_id',$group_user_id)->get();
        // $all_workout_user_id = [];
        // foreach($group_detail as $workout_user){
        //    $all_workout_user_id[] = $workout_user['user_id'] ;
        // }
        // print_r($all_workout_user_id);
        // die;
         $groupmember =DB::table('teamingup_group_users')
        ->leftjoin('users','teamingup_group_users.user_id', '=', 'users.id')
         ->select('users.id')->where('teamingup_group_users.teaming_group_id',$group_user_id)->Where('users.status', 1)->Where('users.role_id', 1)->Where('users.id','!=', Auth()->user()->id)->groupBy('teamingup_group_users.user_id')
        ->get();
        // print_r($groupmember);
        // die;
        if($groupmember!=""){
