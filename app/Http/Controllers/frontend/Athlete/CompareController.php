...
class CompareController extends Controller
{
    ...
    public function updateComparison(Request $request,$id)
    {
        ...
        if($group_user_id!=""){
            ...
            for($i=0; $i<count($group_user_id); $i++){
                $compare_user_exist = Compare::where('user_id', Auth()->user()->id)->where('compare_user_id', $group_user_id[$i])->where('comparison_group_id', $comparisonId)->get();
    //    echo "<pre>";
    //    print_r($compare_user_exist);
    //    die;

                if(count($compare_user_exist) == 0){
                    $groupData= array(
    ...
    public function comparisonGroupAdd(Request $request)
    {
        ...
        // echo "<pre>";
        // print_r($teamingWorkout_id);
        // echo count($teamingWorkout_id);
        // die;
        ...
        // die;
         $groupmember =DB::table('teamingup_group_users')
        ->leftjoin('users','teamingup_group_users.user_id', '=', 'users.id')
         ->select('users.id')->where('teamingup_group_users.teaming_group_id',$group_user_id)->Where('users.id','!=', Auth()->user()->id)->groupBy('teamingup_group_users.user_id')
        ->get();
        // echo "<pre>";
        // print_r($groupmember);
        // die;
