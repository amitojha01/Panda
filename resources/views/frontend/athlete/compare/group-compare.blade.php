					...
					<div class="table-responsive">
						<table class="tabtable" cellpadding="0" cellspacing="0" width="100%">
							<tr>
								...
								{{-- <th>% Rank</th> --}}
								<th class="<?= $segment2 == 'aci_index' ? 'talehighlightedcompare' : '' ; ?>" ><a  href="{{ url('athlete/comparison-details/'.$segment1.'/aci_index') }}">Aci Index</a></th>
								<?php
								$flag = 0;
								$arr = [] ; 
								if($workout_librarys):
								...
								?>
								<th class="<?= $segment2 == $workout_librarys1['Workout_category_library_id'] ? 'talehighlightedcompare' : '' ; ?>" ><a href="{{ url('athlete/comparison-details/'.$group_id.'/'.$workout_librarys1['Workout_category_library_id']) }}"><span>{{ $workout_librarys1['workout_category_title'] }}</span>{{ $workout_librarys1['title'] }}</span></th>	
...
$(document).ready(function(){
            ...
            $('.btn-delete-row').on('click', function(){
               let id = $(this).data('id');
                swal({
                    title: "Are you sure you want to remove this athlete?",
                    icon: "warning",
                    buttons: true,
                    dangerMode: true,
                    })
