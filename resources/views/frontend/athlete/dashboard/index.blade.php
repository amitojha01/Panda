        ...
        <div class="dashbottom">
            ...
            <div class="tab-menu03">              
                <ul>
                   ...
                   <li><a href="#" class="tab-a03 {{$key == 0 ? 'active-a03' : '' }}" data-id="tab{{$value['id']}}" onclick="setStorage(<?= $value['id'] ?>)">{{$value['content_title']}}</a></li>
                    <?php
                    }    
                    ?>
                ...
                <div class="row">
                    ...
                    <div class="col-md-6 col-sm-12 col-xs-12">
                        <div class="workoutboxnew">
                            ...
                            $measurement_value =  DB::table('workout_library')->select('measurement_id')->where('id', $library['id'])->first();
                        // echo "<pre>";
                        //     print_r($measurement_value); die;
                        if($measurement_value->measurement_id == 2 || $measurement_value->measurement_id == 15 ){
                            $userWorkoutExerciseshight = DB::table('user_workout_exercise_logs')->where('workout_library_id', $library['id'])->where('category_id', $library['category_id'])->where('user_id', Auth()->user()->id)->where('unit_1', '>' , 0)->min('unit_1');
                            ...
                            <h6>Last Updated 
                                ...
                                <div class="vidoethumb">
                                    ...
                                    $videoId = $matches[1]; 
                                    ?>
                                    <a href="<?= $recent_video->video ? $recent_video->video : ''; ?>"><img src="{{ 'https://img.youtube.com/vi/' . $videoId . '/mqdefault.jpg' }}" width="85%" height="100%" alt="videoimg" /></a>
        ...
        <div class="col-md-4 col-sm-12 col-xs-12">
            <div class="myprofile">
                ...
                <div class="profiledetails_l">                        
                    ...
                    <div class="athletic_biobox">
                        <h6>Enter Bio, Info, And Awards</h6>
                        <?php $bio= Auth()->user()->bio;  ?>
                         <?php if($bio==""){?>
                        <a href="javascript:void(0)" onclick="bioText()" id="" class="postbtn">CLICK HERE TO ENTER</a>
                         <?php }else{?>
                            <a href="javascript:void(0)" onclick="bioText()" id="" class="postbtn">Click Here to Edit Content</a>

                        <?php } ?>
                        
                        <div class="clr"></div>
                        <form method="post" action="{{ route('athlete.bio.update') }}" enctype="multipart/form-data">
                        @csrf

                        
                        <div id="bioEditor" style="display:none">                      

                            <textarea name="bio" id="bio_txt" placeholder="Type here..."><?= @$bio ?></textarea>
                            <?php if($bio==""){?>

                            <button type="submit" class="biobtn">Save</button>
                        <?php }else{?>
                            <button type="submit" class="biobtn">Update</button>

                        <?php } ?>
                            

                            <a href="javascript:void(0)"  class="biobtn" onclick="cancelBio()">Cancel</a>
                        </div>
                    </form>
