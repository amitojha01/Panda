                ...
                <div class="chat_l">
                  <div class="chat_l_search" style="background:#000;display: flex;align-items: center;justify-content: space-between;">
                    <div style="width:85%;background: white;border-radius: 5px;">
                      <form id="searchchat">
                      <input type="text" id="search_value" name="search" placeholder="Search Conversations" autocomplete="off"/>
                      <button type="button" class="chatbtn"></button>
                      </form>
                    </div>
                    <div style="background: #FFD500; border-radius: 5px;margin-right: -10px;">
                      <a href="{{ url('athlete/connections')}}" style="color: #fff; font-size: 27px; padding: 2px 12px; font-weight: bold; text-decoration: none;">+</a>
                    </div>
                    <div class="clr"></div>
                  </div>
                  <div class="chatlist">
...
<script>
  ...
  function all_chat_member()
  {
    $.ajax({
                ...
                url   : "{{ route('athlete.all-chat-ids') }}",
                data  : '',
                cache: false,
                contentType: false,
                processData: false,
  ...
  var intervalId = window.setInterval(function(){
    ...
    all_chat_member();
    // $('.chat_get').animate({
    //     scrollTop: $('.chat_get .textchat1:last-child').position().top
    // }, 'slow');
  ...
  function view_message_chat(chat_id){
    // const theElement = document.getElementById('chat_get');
    //                 scrollSmoothToBottom(theElement);
    $.ajax({
          type  : "POST",
          headers: {'X-CSRF-TOKEN': $('meta[name="csrf-token"]').attr('content')},
          url   : "{{ route('athlete.view-chats') }}",
          data  : {
            "_token": "{{ csrf_token() }}",
            chatid:chat_id
          },
    ...
    $(document).ready(function(){
      ...
      $(document).on('submit','#sendmsg1',function(e){
        ...
        $.ajax({
                ...
                url   : "{{ route('athlete.save-connections-message') }}",
                data  : formdata,
                dataType : "JSON",
                cache: false,
                contentType: false,
