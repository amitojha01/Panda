            ...
            <div class="tab-container">
                <div class="tab-menu">
                    <div class="searchpanelbox">
                        ...
                        <div class="filter" id="coachfiltersec" ><a href="javascript:void(0);" data-toggle="modal"
                    ...
                    <ul>
                        ...
                        <li><a href="#" class="tab-a <?php if($tabId==2){?>active-a<?php }?> " data-id="tab2" onclick="checkTab(2)">Search Coaches</a></li>  
                        <li><a href="#" class="tab-a <?php if($tabId==3 || $tab_name=="accept"){?>active-a<?php }?> " data-id="tab3" onclick="checkTab(3)">People I Follow</a></li>
                    ...
                    <div class="row createteamingup connectionbox" id="myList">
                        ...
                        <div class="col-sm-3">
                            ...
                            if(count($is_follow) >0 || @$athlete[$i]['profile_type']==0){ ?>
                             ...
                             <a href="{{ url('athlete-profile/'.$athlete[$i]->id.'/'.'athlete') }}" >
                             <?php }else{                                        
                            ...
                            <?php 
                            $exit_follower_user = App\Models\Follower::wherein('status', [1,2])->where('user_id', Auth()->user()->id)->where('follower_id',$athlete[$i]->id)->first();	?>
                            @if($exit_follower_user)
                            <input type="button" id="followers" data-athleteid="{{$athlete[$i]->id}}"
                            class="added_athbtn disable_btn followers" value="Follow" disabled="" style="background: #dd6565; "/>
                            @else
                            <input type="button" id="followers" data-athleteid="{{$athlete[$i]->id}}"
...
<div class="tab <?php if($tabId==2){?>tab-active<?php }?>" data-id="tab2">
            ...
            for($i=0; $i<count($coach); $i++) {
                ...
                <div class="col-sm-3">
                    ...
                    if(count($is_follow) >0 || @$coach[$i]['profile_type']==0){ ?>
                        ...
                        <a href="{{ url('coach-profile/'.$coach[$i]['id'].'/'.'athlete') }}" >
                        ...
                        <?php 
                        $exit_follower_user = App\Models\Follower::wherein('status', [1,2])->where('user_id', Auth()->user()->id)->where('follower_id',$coach[$i]->id)->first();    ?>
                        @if($exit_follower_user)
                        <input type="button" id="followers" data-athleteid="{{$coach[$i]->id}}"
                        class="added_athbtn followers" value="Follow" disabled="" style="background: #dd6565; " />
                        @else
                        <input type="button" id="followers" data-athleteid="{{$coach[$i]->id}}"
                        class="added_athbtn followers" value="Follow" />
                        @endif
    ...
    <?php } ?>
                ...
                <!---Following--->
                <div class="tab <?php if($tabId==3 || $tab_name=="accept"){?>tab-active<?php }?>" data-id="tab3">                   
                        ...
                        if(count($following)>0){
                            ...
                            <div class="col-sm-3">
                                <div class="addathebox"> 
                                    ...
                                    <input type="button" 
                                    class="added_athbtn" onclick="unfollow(<?= $following[$i]->id ?>)" value="Unfollow" />
                                </div>
                            </div>
                        <?php } }else{ ?>
                            <h6>No Result Found!!</h6>
    ...
    <div class="modal-content">
        <div class="modal-body">
            <button type="button" class="close" data-dismiss="modal" aria-label="Close"> <span
                aria-hidden="true">&times;</span> </button>
                <h5>Apply Filter</h5>
                <form action="{{ route('athlete.search-coach') }}" method="post"  enctype="multipart/form-data">
                    @csrf
                    <div class="tab-container">
                        <div class="filtertabopoup_l">
                            <div class="tab-menu01">
