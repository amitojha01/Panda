                            ...
                            // unset($arr[0]);
                             $arr = array_values($arr);
                               print_r($arr);
                                ...
                                //$c=array_combine($user_lookup,$arr);
                                $new_lookup=array();
                                $j=0;
                                foreach($user_lookup as $key => $new){
                                     $user_lookup[$key]->rdate = $arr[$key];
                                   
                                }
                               //echo '<pre>'; print_r($user_lookup);exit; 

                               //$new_lookup = $user_lookup;  
                             //  echo  $user_lookup[0]->record_date;exit; 
                                ...
                                //foreach($user_lookup as $key => $item_value){ 
                                  ...
                                  foreach ($user_lookup->sortBy('record_date') as $item_value){
                                    $WorkoutExercise =DB::table('user_workout_exercise_logs')
                                    ->select('unit_1', 'record_date')
                                    ->where('user_id', $item_value->user_id )
                                     ->where('workout_library_id', $item_value->workout_library_id )
                                ...
                                //for($i=0;$i<count($arr);$i++){
                                    ...
                                    if($WorkoutExercise->record_date== $arr[0] ){
